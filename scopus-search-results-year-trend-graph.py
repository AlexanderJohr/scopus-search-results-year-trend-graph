...
import matplotlib.ticker as mtick  # Import added here

API_KEY = 'YOUR_API_KEY'

yearRange = range(2005, 2024)
...
def main() -> None:
    ...
    for term, counts in plot_data.items():
        ...
        print(f"Term: {term}, Years: {years}, Counts: {counts_values}")  # Debugging-Zeile
        linestyle = '-' if list(plot_data.keys()).index(term) < 10 else ':'
        plt.plot(years, counts_values, label=f'{term}: {publication_counts[term]["Total"]}', linestyle=linestyle)
    ...
    # Setze die x-Achse auf ganze Zahlen
    ...
    # Optional: Formatierung der y-Achse beibehalten
    ...
    plt.savefig("publication_trend.png", bbox_inches="tight", dpi=300)
